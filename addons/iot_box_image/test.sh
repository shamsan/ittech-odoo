...
echo "Running build_image.sh in directory: ${__dir}"
chmod +x "${__dir}/build_utils/"*.sh
echo "Loading methods from build_utils/methods.sh"
...
source ${BUILD_UTILS_DIR}/download_requirements2.sh "${__dir}"
...
sudo ls -l "${MOUNT_POINT}/etc/init_image.sh"

echo "Copying init_image.sh to the mounted image..."

sudo chmod +x "${MOUNT_POINT}/etc/init_image.sh"
